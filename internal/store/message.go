...
type Message struct {
	...
	message_problem *types.ProblemID
	...
	for i := range columns {
		...
		case message.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field message_problem", values[i])
			} else if value.Valid {
				m.message_problem = new(types.ProblemID)
				*m.message_problem = *value.S.(*types.ProblemID)
			}
		default:
