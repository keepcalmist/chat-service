...
import (
	...
	storechat "github.com/keepcalmist/chat-service/internal/store/chat"
	"github.com/keepcalmist/chat-service/internal/store/enttest"
...
func TestChatServiceSchema(t *testing.T) {
	...
	p, err := client.Problem.Get(ctx, problems[0].ID)
	require.NoError(t, err)
	_ = p

	c, err := client.Chat.Get(ctx, chat.ID) // Check that chat is created.
	require.NoError(t, err)
	assert.Equal(t, chat.ID, c.ID)
