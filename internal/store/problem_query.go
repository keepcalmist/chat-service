...
import (
	...
	"github.com/keepcalmist/chat-service/internal/store/predicate"
...
func (pq *ProblemQuery) QueryChat() *ChatQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, problem.ChatTable, problem.ChatColumn),
		)
...
func (pq *ProblemQuery) QueryMessages() *MessageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, problem.MessagesTable, problem.MessagesColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *ProblemQuery) loadMessages(ctx context.Context, query *MessageQuery, nodes []*Problem, init func(*Problem), assign func(*Problem, *Message)) error {
	...
	for _, n := range neighbors {
		fk := n.message_problem
		if fk == nil {
			return fmt.Errorf(`foreign-key "message_problem" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "message_problem" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
